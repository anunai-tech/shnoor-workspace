...
function Navbar({ onNavigate, scrollToContact }) {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled
        ? isDark ? 'bg-gray-900/95 backdrop-blur-md shadow-lg shadow-black/20' : 'bg-white/95 backdrop-blur-md shadow-lg shadow-black/5'
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-3">
            <button onClick={toggleTheme} title={isDark ? 'Switch to light mode' : 'Switch to dark mode'} className={`p-2 rounded-lg transition-all ${isDark ? 'text-gray-300 hover:bg-white/10' : 'text-gray-600 hover:bg-gray-100'}`}>
              {isDark ? <SunIcon /> : <MoonIcon />}
            </button>
            <button onClick={() => onNavigate('login')} className="px-5 py-2 text-sm font-semibold text-white rounded-lg transition-all duration-200 hover:opacity-90 hover:shadow-md" style={{ background: 'linear-gradient(135deg, #0D9488, #0b8279)' }}>
              Get Started
            </button>
          </div>
        ...
        {menuOpen && (
          <div className={`md:hidden pb-4 border-t ${isDark ? 'border-white/10' : 'border-gray-100'}`}>
            ...
            <div className="flex items-center justify-between px-4 pt-3">
              ...
              <button onClick={() => { setMenuOpen(false); onNavigate('login'); }} className="px-5 py-2 text-sm font-semibold text-white rounded-lg" style={{ background: '#0D9488' }}>
                Get Started
              </button>
            </div>
          </div>
...
function Hero({ onNavigate }) {
  ...
  const bgAlt = isDark ? "bg-gray-800"  : "bg-gray-50";
  const bgAlt2= isDark ? "bg-gray-900"  : "bg-gray-50";
    ...
    <section id="home" className={`min-h-screen flex items-center pt-16 ${bg}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-24 lg:py-32 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold mb-6" style={{ background: "#E0F2F1", color: "#0D9488" }}>
            ...
            <div className="flex flex-wrap gap-4">
              <button onClick={() => onNavigate('login')} className="group flex items-center gap-2 px-7 py-3.5 text-white font-semibold rounded-xl text-base transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5" style={{ background: 'linear-gradient(135deg, #0D9488, #0b8279)' }}>
                Get started free
          ...
          <div className={`rounded-2xl overflow-hidden border ${isDark ? "border-white/10" : "border-gray-200"} ${bgAlt}`}>
            <div className="flex h-72">
              <div className={`w-52 flex-shrink-0 flex flex-col border-r ${isDark ? "border-white/10" : "border-gray-200"} ${bgAlt2}`}>
                <div className={`px-3 py-2 border-b ${isDark ? "border-white/10" : "border-gray-200"}`}>
                  <p className={`text-[10px] font-semibold uppercase tracking-widest ${isDark ? "text-gray-500" : "text-gray-400"}`}>Workspace</p>
                  <p className={`text-sm font-semibold mt-0.5 ${isDark ? "text-white" : "text-gray-900"}`}>Shnoor Team</p>
                </div>

                <div className="mt-1">
                  <p className={`px-3 text-[10px] uppercase mb-0.5 tracking-widest ${isDark ? "text-gray-500" : "text-gray-400"}`}>Shortcuts</p>
                  {[
                    { name: "general", active: true, unread: 0 },
                    { name: "announcements", active: false, unread: 2 },
                    { name: "dev-team", active: false, unread: 5 },
                  ].map(ch => (
                    <div key={ch.name} className={`px-3 py-1 flex items-center justify-between text-sm cursor-pointer ${ch.active ? isDark ? "bg-white/10 text-white font-medium" : "bg-gray-200 text-gray-900 font-medium" : isDark ? "text-gray-400 hover:bg-white/5" : "text-gray-600 hover:bg-gray-100"}`}>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] opacity-50">#</span>
                        {ch.name}
                      </div>
                      {ch.unread > 0 && <span className="text-[10px] bg-red-500 text-white px-1 rounded-full">{ch.unread}</span>}
                    </div>
                  ))}
                </div>

                <div className="mt-2">
                  <p className={`px-3 text-[10px] uppercase mb-0.5 tracking-widest ${isDark ? "text-gray-500" : "text-gray-400"}`}>Direct Messages</p>
                  {[
                    { name: "Sarah M", online: true },
                    { name: "James K.", online: false }
                  ].map(user => (
                    <div key={user.name} className={`px-3 py-1 flex items-center gap-2 text-sm cursor-pointer ${isDark ? "text-gray-400 hover:bg-white/5" : "text-gray-600 hover:bg-gray-100"}`}>
              ...
              <div className="flex-1 flex flex-col">
                <div className={`px-5 py-3 border-b flex items-center justify-between ${isDark ? "border-white/10" : "border-gray-100"}`}>
                  <div className="flex items-center gap-2">
                ...
                <div className="flex-1 px-5 py-4 flex flex-col gap-3 overflow-hidden">
                  {[
                    ...
                    { initials: "JK", name: "James K.", msg: "Deployment pipeline finished, ready for approval.", time: "10:44 AM", color: "#1B3A4B" },
                    { initials: "You", name: "You", msg: "Approving now.", time: "10:45 AM", color: "#D4952D" },
                  ].map((m, i) => (
                    <div key={i} className="flex items-baseline gap-3">
                      <div className="w-6 h-6 rounded-md flex-shrink-0 flex items-center justify-center text-white" style={{ background: m.color, fontSize: "9px", fontWeight: 700 }}>{m.initials}</div>
                ...
                <div className={`px-4 py-3 border-t ${isDark ? "border-white/10" : "border-gray-100"}`}>
                  <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs ${isDark ? "border-white/10 text-gray-500" : "border-gray-200 text-gray-400"}`}>
            ...
            <div className={`px-6 py-4 border-t grid grid-cols-3 gap-4 ${isDark ? "border-white/10" : "border-gray-100"} ${bgAlt2}`}>
              {[
...
function About() {
  ...
  const bgPill = isDark ? "bg-gray-800" : "bg-gray-50";
    ...
    <section id="contact" className={`py-24 ${bgAlt}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="max-w-2xl mx-auto text-center mb-12">
          ...
          <h2 className={`text-4xl font-bold tracking-tight mb-4 ${isDark ? "text-white" : "text-gray-900"}`}>We'd love to hear from you</h2>
          <p className={`text-base ${isDark ? "text-gray-400" : "text-gray-500"}`}>Have questions about Shnoor Workspace? Our team is here to help.</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div className="flex flex-col gap-6">
            <p className={`text-base leading-relaxed ${isDark ? "text-gray-400" : "text-gray-500"}`}>
              Whether you're a startup or an enterprise, we're ready to help you build better collaboration workflows with Shnoor Workspace.
            </p>
              ...
              {infoItems.map(item => (
                <div key={item.label} className={`flex items-center gap-4 p-4 rounded-xl border ${bgCard} ${isDark ? "border-white/10" : "border-gray-100"}`}>
                  <div>
                    ...
                    <div className={`text-sm font-semibold ${isDark ? "text-gray-200" : "text-gray-800"}`}>{item.value}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
          <div className="md:col-span-2">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className={`font-bold text-lg ${isDark ? "text-white" : "text-gray-900"}`}>Shnoor <span style={{ color: "#D4952D" }}>International</span></span>
            </div>
            ...
            <div className="flex gap-3 mt-5">
              {[
                ...
                <a key={s.label} href="#" aria-label={s.label} className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200 hover:-translate-y-0.5 ${isDark ? "bg-white/10 text-gray-400 hover:text-white hover:bg-white/20" : "bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-900"}`}>
          ...
          <div>
            <h4 className={`font-semibold text-sm mb-4 ${isDark ? "text-white" : "text-gray-900"}`}>Navigation</h4>
            <ul className="space-y-2.5">
              {pageLinks.map(item => (
                <li key={item.label}>
                  <button onClick={() => scrollTo(item.id)} className={`text-sm transition-colors hover:underline text-left ${isDark ? "text-gray-400 hover:text-white" : "text-gray-500 hover:text-gray-900"}`}>{item.label}</button>
                </li>
              ))}
            </ul>
          </div>
