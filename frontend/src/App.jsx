...
function ChatApp({ onSignOut, onOpenAdmin }) {
  ...
  // Global always-on DM preview listener — runs whenever the socket is connected.
  // This fires when any DM the user is part of receives a new message, regardless of
  // which view is currently active. It refreshes the sidebar conversation list so
  // the preview text and order stay current without the user having to open the DM.
  useEffect(() => {
    if (!connected) return;
    const cleanup = onDMPreviewUpdated(() => {
      getDMConversations().then(setDmConversations).catch(() => {});
    });
    return cleanup;
  }, [connected]);

  // Global role-change listener — when an admin changes this user's role, the server
  // emits user:role_changed to this user's personal socket room. We refresh the user
  // state from the server so the admin star button and permissions update immediately
  // without requiring a re-login.
  useEffect(() => {
    if (!connected) return;
    const cleanup = onUserRoleChanged(() => {
      refreshUser();
    });
    return cleanup;
  }, [connected]);
  ...
  useEffect(() => {
    ...
    const cleanups = [
      ...
      onMessageEdited((msg) => {
        if (msg.space_id !== activeSpace.id) return;
        setMessages(prev =>
          prev.map(m => m.id === msg.id ? { ...formatMsg(msg), reactions: m.reactions } : m)
        );
      }),
    ...
    return () => {
      leaveSpace(activeSpace.id);
      cleanups.forEach(fn => fn?.());
    };
  }, [activeSpace, activeView]);

  // Socket: DM room listeners — also handles dedup for messages received via auto-join
  useEffect(() => {
  ...
  const handleBackToHome = () => {
    ...
    setActiveView('home'); setIsMaximized(false); setMessages([]);
    setTypingUsers([]);
  };
  ...
  const handleSendMessage = async (text) => {
    ...
    try {
      if (activeView === 'space' && activeSpace) {
        await sendSpaceMessage(activeSpace.id, text);
      } else if (activeView === 'dm' && activeDM) {
        await sendDMMessage(activeDM.id, text);
      }
    } catch {
  ...
  const handleTypingChange = (isTyping) => {
    if (activeView === 'space' && activeSpace) {
      emitTyping('space', activeSpace.id, user.name, isTyping);
    } else if (activeView === 'dm' && activeDMConversationId) {
      emitTyping('dm', activeDMConversationId, user.name, isTyping);
    }
  };

  const chatTitle =
    activeView === 'space' && activeSpace ? activeSpace.name : activeDM?.name || '';
  ...
  return (
    <div className="flex flex-col h-screen w-screen overflow-hidden bg-white">
        ...
        {!isMaximized && (
          <ConversationList
            ...
            onSelectConversation={(item) => {
              if (item.type === 'space') {
                const space = formattedSpaces.find(s => s.id === item.id);
                if (space) handleSelectSpace(space);
              } else {
                const dm = dmUsers.find(d => d.id === item.id);
      ...
      {showChatSettings && (
        <ChatSettingsModal onClose={() => setShowChatSettings(false)} />
      )}
    </div>
  );
}
...
function WorkspaceRoot({ user, onSignOut }) {
  ...
  if (inAdmin && user.role === 'admin') {
    return <AdminApp onBack={() => setInAdmin(false)} />;
  }
  ...
  if (loading) {
    return (
      <div style={{ height: '100vh', background: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 44, height: 44, background: '#fff', borderRadius: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 6 }}>
