...
export default function LeftSidebar({
  ...
  const iconOnlyBtn = {
    ...
    borderRadius: 8, background: 'none', border: 'none', cursor: 'pointer', flexShrink: 0,
  };
  ...
  return (
    ...
    >
      {isOpen ? (
        // full expanded sidebar — all content visible
            ...
            <div style={{ marginBottom: 2 }}>
              ...
              {shortcutsOpen && (
                <div style={{ paddingLeft: 14 }}>
                  <button onClick={() => { onHomeClick(); onCloseMobile?.(); }} style={navItemStyle(activeView === 'home')}
                  ...
                  <button onClick={() => { onMentionsClick(); onCloseMobile?.(); }} style={navItemStyle(activeView === 'mentions')}
                  ...
                  {(dmUsers || []).map(member => {
                    ...
                    return (
                      <button key={member.id} onClick={() => { onSelectDM?.(member); onCloseMobile?.(); }}
                        ...
                        onMouseLeave={e => { if (activeDM?.id !== member.id) e.currentTarget.style.background = 'none'; }}
                      >
                        <div style={{ position: 'relative', flexShrink: 0 }}>
        ...
        // icon-only collapsed view — completely separate layout, no transform tricks
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, padding: '10px 0', overflowY: 'auto', flex: 1 }}>
          {/* new chat */}
          <button onClick={() => { setShowNewChat(!showNewChat); }} title="New chat"
            style={{ ...iconOnlyBtn, color: '#1a73e8' }}
            onMouseEnter={e => e.currentTarget.style.background = 'var(--ws-hover)'}
            onMouseLeave={e => e.currentTarget.style.background = 'none'}
          >
          ...
          <div style={{ width: 32, height: '0.5px', background: 'var(--ws-border)', margin: '4px 0' }} />

          {/* home */}
          <button onClick={() => { onHomeClick(); }} title="Home"
            style={{ ...iconOnlyBtn, color: activeView === 'home' ? '#1a73e8' : 'var(--ws-text-muted)', background: activeView === 'home' ? 'rgba(26,115,232,0.1)' : 'none' }}
            onMouseEnter={e => { if (activeView !== 'home') e.currentTarget.style.background = 'var(--ws-hover)'; }}
            onMouseLeave={e => { if (activeView !== 'home') e.currentTarget.style.background = 'none'; }}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {/* mentions */}
          <button onClick={() => { onMentionsClick(); }} title="Mentions"
            style={{ ...iconOnlyBtn, color: activeView === 'mentions' ? '#1a73e8' : 'var(--ws-text-muted)', background: activeView === 'mentions' ? 'rgba(26,115,232,0.1)' : 'none' }}
            onMouseEnter={e => { if (activeView !== 'mentions') e.currentTarget.style.background = 'var(--ws-hover)'; }}
            onMouseLeave={e => { if (activeView !== 'mentions') e.currentTarget.style.background = 'none'; }}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {/* DM avatars (max 5) */}
          {(dmUsers || []).slice(0, 5).map(member => {
            const isOnline = onlineUsers.has(member.id);
            return (
              <button key={member.id} onClick={() => onSelectDM?.(member)} title={member.name}
                style={{ ...iconOnlyBtn, padding: 6, background: activeDM?.id === member.id ? 'rgba(26,115,232,0.1)' : 'none' }}
                onMouseEnter={e => { if (activeDM?.id !== member.id) e.currentTarget.style.background = 'var(--ws-hover)'; }}
                onMouseLeave={e => { if (activeDM?.id !== member.id) e.currentTarget.style.background = 'none'; }}
              >
                <div style={{ position: 'relative' }}>
                  <Avatar initials={member.initials} color={member.color} size={26} avatarUrl={member.avatar_url} />
                  {isOnline && <div style={{ position: 'absolute', bottom: -1, right: -1, width: 8, height: 8, background: '#10B981', borderRadius: '50%', border: '1.5px solid var(--ws-sidebar)' }} />}
                </div>
              </button>
            );
          ...
          {(allSpaces?.length > 0) && <div style={{ width: 32, height: '0.5px', background: 'var(--ws-border)', margin: '4px 0' }} />}

          {/* Space icons (max 5) */}
          {(allSpaces || []).slice(0, 5).map(space => (
            <button key={space.id} onClick={() => onSelectSpace?.(space)} title={space.name}
              style={{ ...iconOnlyBtn, color: activeSpace?.id === space.id ? '#1a73e8' : 'var(--ws-text-muted)', background: activeSpace?.id === space.id ? 'rgba(26,115,232,0.1)' : 'none' }}
              onMouseEnter={e => { if (activeSpace?.id !== space.id) e.currentTarget.style.background = 'var(--ws-hover)'; }}
              onMouseLeave={e => { if (activeSpace?.id !== space.id) e.currentTarget.style.background = 'none'; }}
            >
